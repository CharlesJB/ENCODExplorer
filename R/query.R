...
queryEncode <- function(df = NULL, set_accession = NULL, assay = NULL, 
         ...
         is.null(target) && is.null(treatment) && is.null(project))
    {
        ...
        if((nrow(s1) + nrow(s2)) == 0) {
            warning_message <- "No result found in encode_df. You can try the <searchEncode> function or set the fixed option to FALSE"
            ...
            if(length(dataset_type)) {
                warning_message <- (paste0(warning_message, 
                                                                     ...
                                                                     dataset_type, 
                                                                     ", which is not supported by the current version of ENCODExplorer. You can still recover the list of the associated files using the <resolveEncodeAccession> function "))
            }
