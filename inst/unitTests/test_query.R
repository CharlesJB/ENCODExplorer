...
test.fixed_option <- function() {
  obs = tryCatch(query(target = "rabbit iGG", fixed=T),error=function(e) e, warning=conditionMessage)
  exp = "No result found. You can try the <search> function or set the fixed option to FALSE"
...
test.query_4_dataset <- function() {
  res_dataset_only = query(set_accession = "ENCSR685AIQ")
...
test.combined_query_on_custom_df <- function() {
  ...
  res_combo_custom = query(df = small_encode_df, target = "ELAVL1-human", 
...
test.query_revoked_file <- function() {
  res_revok = query(assay = "chipseq", biosample = "mcf7", fixed = F, 
                    file_status = "revoked")
