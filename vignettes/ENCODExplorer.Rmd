...
experiment).] |The dataset accession|
...
*CEL*,*csqual*,*sam*,*wig*,*gff*,*fasta*,*tagAlign*]|The file format|
|lab|The laboratory|
...
downloadEncode(file_acc=query_results$file_accession, dt = encode_df)
```

2) specify the format to the `downloadEncode` function

```{r collapse=TRUE, eval=FALSE}
downloadEncode(file_acc = search_results$accession, dt=encode_df,
                format = "bed")
```
...
### shinyEncode
This function is the shinyApp of ENCODExplorer that implement the queryEncode 
and the fuzzySearch function. The createDesign and downdloadEncode function can also be
use on the search results.
Files related with the shiny app can be found at ENCODExplorer/inst/shiny