...
For some reason, some accession number can be missing for non-experiment 
datasets. The metadata retrieval is not completely available in the current 
release of ENCODExplorer. So that the user can at least have the list of 
associated files, we implemented a new function `resolveEncodeAccession` which 
return a `data.frame` containing the list of files associated to a particular 
accession number.

```{r files, collapse=TRUE}
related_files <- resolveEncodeAccession(accession = 'ENCSR361ONJ')
related_files$files
```
