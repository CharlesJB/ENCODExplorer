...
|---------|------------|-------------|
|accession|experiment / dataset|The experiment or dataset accession|
...
query_results <- query(assay = "RNA-seq", organism = "Homo sapiens",
...
query_results <- query(assay = "rnaseq", organism = "Homo sapiens",
                       biosample = "MCF-7", file_format = "fastq", fixed = TRUE)
```
...
The `search` function returns a data frame which corresponds to the result page
...
search_results <- search(searchTerm = "a549 chip-seq homo sapiens",
                         limit = "all")
```
...
query_results <- query(assay = "switchgear", target ="elavl1", fixed = FALSE)
```
...
query_results <- query(assay = "switchgear", target ="elavl1",
...
both. When using **accession**, you will get all the files directly linked with
