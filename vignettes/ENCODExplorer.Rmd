...
Éric Fournier, Charles Joly Beauparlant, Audrey Lemacon, Louis Gendron and
Arnaud Droit.
...
To use the functionalities of the ENCODExplorer package, you first need to
download the `data.table` containing all the metadata from ENCODE.

This `data.table` is available through the AnnotationHub package. For
convenience, we provide the following function to quickly extract the metadata:
...
searches on the `encode_df_lite` object. The character vector or the list of 
...
in which to search for the query term by using the `filterVector` parameter. 
...
By default, ENCODExplorer retrieves the ENCODE metadata from its sister package,
[ENCODExplorerData]
(https://www.bioconductor.org/packages/bioc/html/ENCODExplorerData.html). The
metadata provided by [ENCODExplorerData]
(https://www.bioconductor.org/packages/bioc/html/ENCODExplorerData.html)
is updated with each Bioconductor 
release. However, since all of ENCODExplorer's function take an explicit `df`
parameter, it is possible to use [ENCODExplorerData]
(https://www.bioconductor.org/packages/bioc/html/ENCODExplorerData.html)
functionality to generate an up-to-date data.table, and pass it to 
`ENCODExplorer`'s functions.

We refer the user to the [ENCODExplorerData]
(https://www.bioconductor.org/packages/bioc/html/ENCODExplorerData.html)
vignettes for details on how to generate an up-to-date data.table.
