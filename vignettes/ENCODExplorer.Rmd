...
query_results <- queryEncode(df=get_encode_df(), organism = "Homo sapiens",
...
query_results <- queryEncode(df=get_encode_df(), assay = ".*RNA-seq",
...
query_results <- queryEncodeGeneric(df = encode_df, biosample_name="HeLa-S3",
...
`fuzzySearch` or `queryEncode` (i.e. within `get_encode_df()`), the user can access the 
...
the same design as `get_encode_df()`. This format contains more metadata and allow the 
...
query_results <- queryEncode(df=get_encode_df(), assay = "switchgear", target ="elavl1", fixed = FALSE)
```
...
downloadEncode(query_results, df = get_encode_df())
```
