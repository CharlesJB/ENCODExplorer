...
of ENCODE files metadata: `encode_df`. This database contain every files from all
dataset type.
...
To following request will produce a data.table with every files that contain
the term brca.
...
downloadEncode(df=encode_df, resultSet = query_results, resultOrigin = "queryEncode")
```

2) specify the format to the `downloadEncode` function

```{r collapse=TRUE, eval=FALSE}
downloadEncode(df=encode_df, resultSet = search_results,
               resultOrigin = "searchEncode", format = "bed")
```
