...
in the `query` and `download` function by setting the *df* option of those 
functions.

```{r query, eval=FALSE}
query_results <- query(df = new_encode_df, assay = "switchgear", target ="elavl1", file_format = "bed_broadPeak" , fixed = F)
download(df = new_encode_df, resultSet = query_results, resultOrigin = "query")
```

Be sure to use the same referencial *df* for both **query** and **download**.
